...
The v1 library has a read-only system scope and a workspace-owned scope. A workspace draft can be edited through an interactive UI; publish creates a new immutable version. Cross-workspace or organization-wide sharing is deferred because current graph, source, and authorization authority is workspace-scoped. Display names and a UI's `latest` suggestion are never execution identity. Published versions cannot be edited in place.
...
The later backend API needs: paginated/searchable operator catalog with support/gate diagnostics; authorized system/workspace component list and exact version read; workspace draft save with a revision/compare-and-swap token for safe autosave; **transient draft validation and bounded sample preview** for the current unsaved editor state; immutable publish with version conflict; exact component resolution during graph save/compile/preview; and explicit dry-run upgrade plus normal GraphRevision append. These are contract needs, not routes created by PL-C0. Interactive validation returns typed step/field diagnostics and projected schema without publishing or saving a graph. Sample preview requires an explicitly authorized source and returns the same definition/revision or request token submitted by the editor, so the UI can discard an older response after a newer edit. The UI cancels obsolete requests; the service propagates cancellation and applies the normal Preview deadline, row/byte/source-scan and concurrency limits. Autosave conflicts surface to the editor rather than overwriting another draft revision. A preview failure or cancellation leaves the draft, graph, PlanVersion, Run, and Artifact unchanged. Authentication and workspace scope are checked before component/source resolution. A recipe contains no connector identity, credential, file path, source row, or secret reference.

The UI implementation belongs in the OpenShip repository, after the StillFlow wire contract and API exist. The recipe editor is the primary authoring surface: users search an approved operation, add/reorder/remove a step, change a typed argument, and immediately see current-schema/field diagnostics and an optional bounded sample preview. A stale validation/preview response must never replace results for a newer edit; unavailable or deferred operations show a reason in the editor. The editor may debounce requests and autosave drafts through the revision token, with visible saving/conflict state; **publishing is always an explicit action** that freezes a new version. The canvas picker places that published version, and the instance inspector edits only its public parameters and shows a version-upgrade diff. The UI must not implement recipe execution, source authorization, or Polars semantics in browser code. Existing OpenShip #36 covers older node categories; OpenShip #37 owns this interactive library UI. PL-C0 does not mutate that repository.
...
| Interactive authoring | Transient unsaved validation/preview, stale-response identity, cancellation, explicit publish, and no preview side effects are specified in §9 |
