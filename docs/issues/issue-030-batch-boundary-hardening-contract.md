...
> Risk: High
> Issue: #30
> Authorized base: `main@910489d4ef6bdc1e3db6d8ba9e9a68a3078f20ca`
> Last updated: 2026-08-12

## 1. Objective

Harden the version 1 logical-schema and `BatchEnvelope` boundary before local
connector #6 and API/engine work depend on it. Close the request-cancellation
...
Snapshot storage #28/#29 is already accepted on the authorized base. This
correction must preserve every storage contract and keep storage source files
unchanged.

## 2. Source policy

- The latest accepted `main` named above is the only implementation base.
- Historical PR #31 and its branch are read-only references.
- The replacement implementation must not merge, rebase, or cherry-pick any
  historical branch.
- The full storage test suite must remain green without storage-source changes.

## 3. Authorized public changes
...
The exact accessor names may follow existing style, but construction and
behavior must match section 6.

### Behavioral tightening

Previously accepted schemas above the new limits and non-canonical Arrow
metadata representations become invalid. This is an intentional validation
tightening, not an envelope-version or logical-type-version change. No
compatibility shim is provided for invalid or non-canonical inputs.
...
## 5. Schema resource laws

### 5.1 Bounded iterative validation

`LogicalType::validate`, `LogicalField::validate`, and
`LogicalSchema::validate` must enforce the same limits. Validation of nested
`List` and `Struct` shapes must use an explicit work stack rather than one Rust
call-stack frame per nesting level.
...
- an over-limit input returns the matching typed error before Arrow conversion
  or fingerprint serialization.
...
Existing `BatchEnvelope::try_new` and `try_from_parts` remain source compatible
and may create a one-shot factory internally.

## 7. Strict canonical Arrow decoding
...
- schema or field metadata JSON whose input bytes differ from compact JSON of
  the decoded ordered map;
...
## 8. Cancellation and deadline linearization
...
- Every new error contains only depth, field, text-byte, or metadata-scope
  information.
- No error contains row values, payload debug output, credentials, SQL,
  locators, paths, or raw metadata values.
- Schema and Arrow validation return typed errors; they do not panic.
- No production `unwrap`, `expect`, unchecked index, unsafe block, or recursion
  proportional to external schema depth is authorized.
- Fingerprints remain non-cryptographic comparison indexes.

## 11. Explicit non-goals

- Any change to SQLite, Parquet, snapshot manifests, publication, recovery,
  retention, tombstones, or GC.
...
- Exactly 4,096 total logical fields validate; field 4,097 returns the
  field-limit error.
- Exactly 1 MiB of counted schema text validates; one additional byte returns
  the text-limit error.
...
- Canonical logical -> Arrow -> logical round trips remain equal for the
...
- An inner stream that cancels its token and returns a valid envelope in the
  same poll yields one `Cancelled` error and no envelope.
...
- All `stillflow-storage` tests remain green without storage-source changes.
