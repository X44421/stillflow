...
use stillflow_core::{
    BatchEnvelope, ConnectorError, ConnectorResult, ErrorCategory, PreviewData, PreviewRequest,
};
...
pub(crate) async fn preview_asset(
    ...
    while let Some(envelope) = reader.next_envelope().await? {
        ...
        let output = BatchEnvelope::try_new(
...
mod tests {
    ...
    #[test]
    fn fitting_prefix_uses_compacted_variable_width_buffers() {
        ...
        let batch = RecordBatch::try_new(
            ...
            vec![
                ...
                Arc::new(StringArray::from(vec!["a", "b", "x".repeat(4096).as_str()])),
            ],
        )
        .expect("batch");
        let two = compact_range(&batch, 0, 2).expect("two rows");
        let fit = largest_fitting_prefix(&batch, two.get_array_memory_size()).expect("fit");
        assert_eq!(fit, 2);
        assert!(two.get_array_memory_size() < batch.get_array_memory_size());
    }
}
