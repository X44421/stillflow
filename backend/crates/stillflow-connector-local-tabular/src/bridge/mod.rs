...
use stillflow_core::{
    logical_schema_to_arrow, ConnectorError, ConnectorResult, ErrorCategory, LogicalField,
...
pub(crate) fn dataframe_to_record_batch(
    ...
    let target_schema = logical_schema_to_arrow(logical_schema).map_err(|_| {
        bridge_error(
            ErrorCategory::Internal,
            "the canonical Arrow schema could not be constructed",
        )
    })?;
    if logical_schema.fields.is_empty() {
        let options = RecordBatchOptions::new().with_row_count(Some(height));
        return RecordBatch::try_new_with_options(target_schema, Vec::new(), &options).map_err(
            |_| {
                bridge_error(
                    ErrorCategory::Internal,
...
mod tests {
    ...
    use stillflow_core::{ColumnId, LogicalField, LogicalType};
    use uuid::Uuid;
    ...
    fn imports_null_and_variable_width_arrays_and_releases_on_drop() {
        ...
        .expect("frame");
        let batch = dataframe_to_record_batch(frame, &schema()).expect("bridge");
    ...
    fn imports_empty_frames() {
        ...
        .expect("empty frame");
        let batch = dataframe_to_record_batch(frame, &schema()).expect("empty bridge");
    ...
    fn imports_sliced_frames_and_supports_immediate_drop() {
        ...
        .expect("frame")
        .slice(1, 2);
        let batch = dataframe_to_record_batch(frame, &schema()).expect("sliced bridge");
        ...
        drop(dataframe_to_record_batch(frame, &schema()).expect("early-drop bridge"));
    }
    ...
    fn imports_chunked_frames_without_changing_row_order() {
        ...
        let batch = dataframe_to_record_batch(frame, &schema()).expect("chunked bridge");
        let ids = batch
            .column(0)
            .as_any()
            .downcast_ref::<Int64Array>()
    ...
    fn rejects_nulls_in_required_fields() {
        ...
        let error = dataframe_to_record_batch(frame, &required).expect_err("required null");
