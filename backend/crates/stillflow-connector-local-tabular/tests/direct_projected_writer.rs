//! E24-JSON-A2 production differential oracle for issue #158, converted to a
//! RUNTIME differential by O1-J1 (issue #296).
//!
//! The projected-row routing is no longer a compile-time feature: every read
//! selects its arm through the per-connection `jsonDirectProjectedWriter`
//! config key (`false` = generic DOM path, `true` = raw-slice direct
//! projected assembly). Every test therefore exercises BOTH arms at runtime
//! on identical fixture bytes and asserts identical observable behavior
//! (final tabular values as extracted from the public envelopes, error
//! categories, stable messages, earliest failing row). Any mismatch is a
//! semantic reject of the direct path (contract section 4 of
...
//! both arms must emit the declared epoch unit to the retained Polars
//! JsonReader, with no post-parse compensation.
...
fn connection(root: &std::path::Path) -> SourceConnection {
    connection_with_mode(root, false)
}

/// Builds a fixture-root connection with the O1-J1 runtime routing knob:
/// `direct = true` routes every JSON row through the direct projected
/// assembler, `direct = false` keeps the generic DOM path (production
/// default).
fn connection_with_mode(root: &std::path::Path, direct: bool) -> SourceConnection {
...
/// O1-J1 runtime differential comparator.
///
/// Ignores the per-inspect random identity metadata (`stillflow.column.id`
/// UUIDs and the derived schema fingerprint are regenerated by every inspect
/// call, so a fresh inspect per routing arm cannot match on them) and compares
/// the observable structure instead: field names/types/nullability, per-column
/// array data, row counts, and exact error category + message.
fn outcomes_equivalent(left: &[Outcome], right: &[Outcome]) -> bool {
    left.len() == right.len()
        && left.iter().zip(right.iter()).all(|(a, b)| match (a, b) {
            (Ok(batch_a), Ok(batch_b)) => {
                let (schema_a, schema_b) = (batch_a.schema(), batch_b.schema());
                schema_a.fields.len() == schema_b.fields.len()
                    && schema_a.fields.iter().zip(schema_b.fields.iter()).all(
                        |(field_a, field_b)| {
                            field_a.name() == field_b.name()
                                && field_a.data_type() == field_b.data_type()
                                && field_a.is_nullable() == field_b.is_nullable()
                        },
                    )
                    && batch_a.num_columns() == batch_b.num_columns()
                    && batch_a.num_rows() == batch_b.num_rows()
                    && batch_a
                        .columns()
                        .iter()
                        .zip(batch_b.columns().iter())
                        .all(|(column_a, column_b)| column_a == column_b)
            }
...
///
/// O1-J1 (#296) runtime differential: the case runs once per routing arm
/// (generic DOM path, direct projected assembler) on identical fixture bytes
/// and the observable outcomes must match exactly; the generic arm's outcomes
/// are returned.
async fn run_case(
    lines: &[&str],
    ...
    let mut first_mode: Option<Vec<Outcome>> = None;
    for direct in [false, true] {
        ...
        let connection = connection_with_mode(temp.path(), direct);
        let outcomes = drain_asset_with_context(
            &connection,
            "case.ndjson",
            override_types,
            projection_indices,
        ...
        match &first_mode {
            Some(previous) => assert!(
                outcomes_equivalent(&outcomes, previous),
                "runtime differential mismatch between the generic (knob off) \
                 and direct projected (knob on) arms\nknob-off: {previous:?}\n\
                 knob-on: {outcomes:?}"
            ),
            None => first_mode = Some(outcomes),
        }
    }
    first_mode.expect("both routing arms ran")
}
...
async fn pretty_printed_array_rows_with_newlines_inside_selected_values() {
    ...
    // O1-J1 runtime differential: the direct projected arm must reproduce the
    // same rows on the same fixture.
    let direct_outcomes = drain_asset(
        &connection_with_mode(temp.path(), true),
        "case.json",
        Some(&types),
        Some(&[0, 1, 2]),
        4,
    ...
    .await;
    assert!(
        outcomes_equivalent(&direct_outcomes, &outcomes),
        "direct projected arm diverged on pretty-printed array document"
    );
...
async fn cancellation_mid_stream_preserves_the_public_surface() {
    // O1-J1 runtime differential: the cancellation surface must hold on both
    // routing arms.
    for direct in [false, true] {
        let types = abcd_types();
        let temp = TempDir::new().expect("temporary fixture root");
        ...
        // arms: every row/batch checkpoint runs the shared ensure_active gate.
        cancellation.cancel();
        let second = stream
            .next()
            .await
...
async fn expired_deadline_fails_closed_identically() {
    ...
    // O1-J1 runtime differential: the fail-closed open surface is identical
    // on the direct projected arm.
    let direct_error = try_open_stream_with_context(
        &connection_with_mode(temp.path(), true),
        "case.ndjson",
        Some(&types),
        Some(&[0]),
        4,
        RequestContext::with_deadline(
            tokio::time::Instant::now() - std::time::Duration::from_secs(1),
        ),
    )
    .await
    .err()
    .unwrap_or_else(|| panic!("expired deadline must fail the open (direct arm)"));
    assert_eq!(direct_error.category(), ErrorCategory::Timeout);
    assert_eq!(
        direct_error.to_string(),
        error.to_string(),
        "fail-closed open surface diverged between routing arms"
    );
}
...
async fn json_array_shape_document_parity() {
    ...
    .await;
    // O1-J1 runtime differential: direct projected arm must match.
    let direct_outcomes = drain_asset(
        &connection_with_mode(temp.path(), true),
        "case.json",
        Some(&base),
    ...
    assert!(
        outcomes_equivalent(&direct_outcomes, &outcomes),
        "direct projected arm diverged on array document"
    );
...
async fn raw_control_bytes_inside_selected_subtree_are_canonicalized() {
    ...
    // O1-J1 runtime differential: the direct projected arm must canonicalize
    // the control-byte-bearing captures to the identical rows.
    let direct_outcomes = drain_asset(
        &connection_with_mode(temp.path(), true),
        "case.json",
        Some(&types),
    ...
    assert!(
        outcomes_equivalent(&direct_outcomes, &outcomes),
        "direct projected arm diverged on control-byte document"
    );
    assert_eq!(i64_values(&batch, "m"), [Some(0), Some(1), Some(2)]);
    assert_eq!(
        list_struct_values(&batch, "ls"),
