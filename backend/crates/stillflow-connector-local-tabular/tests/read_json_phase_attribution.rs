//! E24-JSON-P0: real-path NDJSON stage attribution.
...
const TIMING_ENV: &str = "STILLFLOW_JSON_PHASE_TIMING";
...
fn connection(root: &Path) -> SourceConnection {
    SourceConnection::try_new(
        ...
        CredentialRef::new("cred://local/e24-json-p0").expect("credential reference"),
    )
    .expect("connection")
}
...
fn set_timing(enabled: bool) {
    if enabled {
        std::env::set_var(TIMING_ENV, "1");
    } else {
        std::env::remove_var(TIMING_ENV);
    }
}
...
fn classify_verdict(
    ...
    if overhead_10.abs() > OVERHEAD_LIMIT_PCT || overhead_100.abs() > OVERHEAD_LIMIT_PCT {
        return "ATTRIBUTION_INVALID_OVERHEAD".to_string();
    }
    if coverage_100 < COVERAGE_FLOOR {
        return "ATTRIBUTION_INCOMPLETE".to_string();
    }
    if largest_share >= DOMINANT_FLOOR {
        let stage = largest_stage
            .strip_prefix("json_phase_")
            .and_then(|name| name.strip_suffix("_ns"))
            .unwrap_or(largest_stage)
            .to_ascii_uppercase();
        return format!("ATTRIBUTION_DOMINANT_{stage}");
    }
    "ATTRIBUTION_MIXED".to_string()
}

#[tokio::test]
#[ignore]
async fn json_phase_attribution_focused() {
    let temp = TempDir::new().expect("temp");
    let root = temp.path();
    let metrics_out: PathBuf = root.join("e24_json_phase.out");
    std::env::set_var(METRICS_OUT_ENV, &metrics_out);
    let cases = [(10_usize, 100_000_usize), (100, 100_000)];
    for (cols, rows) in cases {
        let name = format!("p0_ndjson_{cols}c_{rows}r.ndjson");
        eprintln!("[e24-json-p0] generating {name}");
    ...
    for (cols, rows) in cases {
        ...
        eprintln!("[e24-json-p0] measuring {name}");
        ...
        let mut on_stages: Vec<BTreeMap<String, u64>> = Vec::new();
        let mut off_stages: Vec<BTreeMap<String, u64>> = Vec::new();
        ...
        assert_eq!(off_fp, on_fp, "timing on/off must not change output");
        ...
        for _ in 0..FOCUSED_REPS {
            ...
            off_stages.push(stage_delta(&snapshot, &after));
            snapshot = after;
        }
        ...
        for _ in 0..FOCUSED_REPS {
            ...
            let after = read_counter_snapshot(&metrics_out);
            on_stages.push(stage_delta(&snapshot, &after));
            snapshot = after;
        }
        set_timing(false);
        ...
        let mut stage_medians = BTreeMap::new();
        for label in STAGE_LABELS {
            let samples: Vec<u64> = on_stages
                .iter()
                .map(|row| row.get(*label).copied().unwrap_or(0))
                .collect();
            stage_medians.insert((*label).to_string(), median_u64(&samples));
        }
        let stage_sum_ns: u64 = stage_medians.values().copied().sum();
        ...
        let (largest_label, largest_ns) = stage_medians
            .iter()
        ...
        cell_results.push(serde_json::json!({
            ...
            "off_stage_ns": off_stages,
            "on_stage_ns": on_stages,
            "on_stage_median_ns": stage_medians,
            "coverage": coverage,
