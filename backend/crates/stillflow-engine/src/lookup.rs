...
/// `pub(crate)` since O0-B2-A1-A2 (#163): the A2 incremental mutation layer
/// rederives this table after every accepted mutation (same-call refresh, the
/// accepted A1 stale-state guarantee). Semantics are unchanged.
pub(crate) fn build_ordinals(schema: &LogicalSchema) -> Vec<(ColumnId, u32)> {
