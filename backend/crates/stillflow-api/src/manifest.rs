//! Authoritative route/schema manifest seed.
//!
//! E5-A1 adds operation entries here as typed handlers land. OpenAPI output is
//! derived from this manifest; it is not an independent source of truth.

use serde::Serialize;
use serde_json::Value;

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct RouteSpec {
    pub operation_id: &'static str,
    pub method: &'static str,
    pub path: &'static str,
    pub request_schema: &'static str,
    pub response_schema: &'static str,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct SchemaSpec {
    pub name: &'static str,
    pub version: u16,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct RouteManifest {
    pub api_version: u16,
    pub routes: &'static [RouteSpec],
    pub schemas: &'static [SchemaSpec],
}

const fn route(
    operation_id: &'static str,
    method: &'static str,
    path: &'static str,
    request_schema: &'static str,
    response_schema: &'static str,
) -> RouteSpec {
    RouteSpec {
        operation_id,
        method,
        path,
        request_schema,
        response_schema,
    }
}

const fn schema(name: &'static str) -> SchemaSpec {
    SchemaSpec { name, version: 1 }
}

pub const E5_A1_ROUTES: &[RouteSpec] = &[
    route(
        "handshake",
        "POST",
        "/v1/handshake",
        "HandshakeRequest",
        "HandshakeResponse",
    ),
    route(
        "health.liveness",
        "GET",
        "/v1/health/live",
        "EmptyRequest",
        "HealthView",
    ),
    route(
        "health.readiness",
        "GET",
        "/v1/health/ready",
        "EmptyRequest",
        "HealthView",
    ),
    route(
        "health.read",
        "GET",
        "/v1/health",
        "EmptyRequest",
        "HealthView",
    ),
    route(
        "metrics.read",
        "GET",
        "/v1/metrics",
        "ObjectIdRequest",
        "MetricsView",
    ),
    route(
        "workspace.create",
        "POST",
        "/v1/workspaces",
        "CreateWorkspaceRequest",
        "WorkspaceView",
    ),
    route(
        "workspace.archive",
        "POST",
        "/v1/workspaces/{workspaceId}/archive",
        "ArchiveWorkspaceRequest",
        "WorkspaceView",
    ),
    route(
        "workspace.read",
        "GET",
        "/v1/workspaces/{objectId}",
        "ObjectIdRequest",
        "WorkspaceView",
    ),
    route(
        "member.create",
        "POST",
        "/v1/members",
        "CreateMemberRequest",
        "MemberView",
    ),
    route(
        "member.read",
        "GET",
        "/v1/members/{objectId}",
        "ObjectIdRequest",
        "MemberView",
    ),
    route(
        "member.revoke",
        "POST",
        "/v1/members/{memberId}/revoke",
        "RevokeMemberRequest",
        "MemberView",
    ),
    route(
        "role.create",
        "POST",
        "/v1/roles",
        "CreateRoleRequest",
        "RoleView",
    ),
    route(
        "role.read",
        "GET",
        "/v1/roles/{objectId}",
        "ObjectIdRequest",
        "RoleView",
    ),
    route(
        "role.capabilities.set",
        "POST",
        "/v1/roles/{roleId}/capabilities",
        "SetRoleCapabilitiesRequest",
        "RoleView",
    ),
    route(
        "member.role.assign",
        "POST",
        "/v1/members/{memberId}/roles",
        "AssignRoleRequest",
        "RoleAssignmentView",
    ),
    route(
        "service-account.create",
        "POST",
        "/v1/service-accounts",
        "CreateServiceAccountRequest",
        "ServiceAccountView",
    ),
    route(
        "service-account.read",
        "GET",
        "/v1/service-accounts/{objectId}",
        "ObjectIdRequest",
        "ServiceAccountView",
    ),
    route(
        "service-account.revoke",
        "POST",
        "/v1/service-accounts/{serviceAccountId}/revoke",
        "RevokeServiceAccountRequest",
        "ServiceAccountView",
    ),
    route(
        "credential.register",
        "POST",
        "/v1/credentials",
        "RegisterCredentialReferenceRequest",
        "CredentialRefView",
    ),
    route(
        "credential.read",
        "GET",
        "/v1/credentials/{objectId}",
        "ObjectIdRequest",
        "CredentialRefView",
    ),
    route(
        "credential.rotate.begin",
        "POST",
        "/v1/credentials/{credentialId}/rotation",
        "BeginCredentialRotationRequest",
        "CredentialRefView",
    ),
    route(
        "credential.rotate.complete",
        "POST",
        "/v1/credentials/{credentialId}/rotation/complete",
        "CompleteCredentialRotationRequest",
        "CredentialRefView",
    ),
    route(
        "credential.revoke",
        "POST",
        "/v1/credentials/{credentialId}/revoke",
        "RevokeCredentialRequest",
        "CredentialRefView",
    ),
    route(
        "credential.recover",
        "POST",
        "/v1/credentials/{credentialId}/recover",
        "RecoverCredentialRequest",
        "CredentialRefView",
    ),
    route(
        "session.create",
        "POST",
        "/v1/sessions",
        "CreateSessionRequest",
        "SessionView",
    ),
    route(
        "session.list",
        "GET",
        "/v1/sessions",
        "ListRequest",
        "ObjectList<SessionView>",
    ),
    route(
        "session.read",
        "GET",
        "/v1/sessions/{objectId}",
        "ObjectIdRequest",
        "SessionView",
    ),
    route(
        "session.close",
        "POST",
        "/v1/sessions/{sessionId}/close",
        "CloseSessionRequest",
        "SessionView",
    ),
    route(
        "connection.test",
        "POST",
        "/v1/connections/test",
        "TestSourceConnectionRequest",
        "ConnectionStatus",
    ),
    route(
        "connection.register",
        "POST",
        "/v1/connections",
        "RegisterSourceConnectionRequest",
        "SourceConnectionView",
    ),
    route(
        "connection.list",
        "GET",
        "/v1/connections",
        "ListRequest",
        "ObjectList<SourceConnectionView>",
    ),
    route(
        "connection.read",
        "GET",
        "/v1/connections/{objectId}",
        "ObjectIdRequest",
        "SourceConnectionView",
    ),
    route(
        "connection.update",
        "POST",
        "/v1/connections/{connectionId}",
        "UpdateSourceConnectionRequest",
        "SourceConnectionView",
    ),
    route(
        "connection.transition",
        "POST",
        "/v1/connections/{connectionId}/transition",
        "TransitionSourceConnectionRequest",
        "SourceConnectionView",
    ),
    route(
        "connection.retire",
        "POST",
        "/v1/connections/{connectionId}/retire",
        "RetireSourceConnectionRequest",
        "SourceConnectionView",
    ),
    route(
        "asset.list",
        "GET",
        "/v1/assets",
        "ListRequest",
        "ObjectList<SourceAssetView>",
    ),
    route(
        "asset.discover",
        "POST",
        "/v1/assets/discover",
        "DiscoverAssetsRequest",
        "Vec<SourceAssetView>",
    ),
    route(
        "asset.inspect",
        "POST",
        "/v1/assets/inspect",
        "InspectAssetRequest",
        "AssetMetadata",
    ),
    route(
        "asset.preview",
        "POST",
        "/v1/assets/preview",
        "PreviewAssetRequest",
        "PreviewView",
    ),
    route(
        "node-types.list",
        "GET",
        "/v1/node-types",
        "EmptyRequest",
        "NodeCatalogView",
    ),
    route(
        "dataset.create",
        "POST",
        "/v1/datasets",
        "CreateDatasetRequest",
        "DatasetView",
    ),
    route(
        "dataset.list",
        "GET",
        "/v1/datasets",
        "ListRequest",
        "ObjectList<DatasetView>",
    ),
    route(
        "dataset.read",
        "GET",
        "/v1/datasets/{objectId}",
        "ObjectIdRequest",
        "DatasetView",
    ),
    route(
        "dataset.archive",
        "POST",
        "/v1/datasets/{objectId}/archive",
        "ObjectIdRequest",
        "DatasetView",
    ),
    route(
        "profile.history.list",
        "GET",
        "/v1/datasets/{datasetId}/profile-history",
        "ListProfileHistoryRequest",
        "ProfileHistoryPageView",
    ),
    route(
        "plan.create",
        "POST",
        "/v1/plans",
        "CreatePlanRequest",
        "PlanView",
    ),
    route(
        "plan.list",
        "GET",
        "/v1/plans",
        "ListRequest",
        "ObjectList<PlanView>",
    ),
    route(
        "plan.load",
        "GET",
        "/v1/plans/{objectId}",
        "ObjectIdRequest",
        "PlanView",
    ),
    route(
        "plan.version.save",
        "POST",
        "/v1/plans/{planId}/versions",
        "SavePlanVersionRequest",
        "PlanVersionView",
    ),
    route(
        "plan.version.list",
        "GET",
        "/v1/plans/{planId}/versions",
        "ListPlanVersionsRequest",
        "ObjectList<PlanVersionView>",
    ),
    route(
        "plan.version.read",
        "GET",
        "/v1/plan-versions/{objectId}",
        "ObjectIdRequest",
        "PlanVersionView",
    ),
    route(
        "plan.version.publish",
        "POST",
        "/v1/plan-versions/{planVersionId}/publish",
        "PublishPlanVersionRequest",
        "PlanVersionView",
    ),
    route(
        "plan.clone",
        "POST",
        "/v1/plans/clone",
        "ClonePlanRequest",
        "PlanVersionView",
    ),
    route(
        "plan.diff",
        "POST",
        "/v1/plans/diff",
        "PlanDiffRequest",
        "PlanDiffView",
    ),
    route(
        "plan.validate",
        "POST",
        "/v1/plans/validate",
        "ValidatePlanRequest",
        "ValidatePlanView",
    ),
    route(
        "engine.preview",
        "POST",
        "/v1/engine/preview",
        "EnginePreviewRequest",
        "EnginePreviewView",
    ),
    route(
        "graph-revisions.save",
        "POST",
        "/v1/graph-revisions",
        "SaveGraphRevisionRequest",
        "GraphRevisionView",
    ),
    route(
        "graph-revisions.read",
        "GET",
        "/v1/graph-revisions/{graphId}",
        "GetGraphRevisionRequest",
        "GraphRevisionView",
    ),
    route(
        "graph-revisions.history",
        "GET",
        "/v1/graph-revisions/{graphId}/history",
        "ListGraphRevisionsRequest",
        "GraphRevisionHistoryView",
    ),
    route(
        "graph-revisions.migrate",
        "POST",
        "/v1/graph-revisions/{graphId}/migrate",
        "MigrateGraphRevisionRequest",
        "GraphRevisionMigrationView",
    ),
    route(
        "node-graph.compile",
        "POST",
        "/v1/node-graphs/compile",
        "NodeGraphCompileRequest",
        "NodeGraphCompileView",
    ),
    route(
        "node-graph.preview",
        "POST",
        "/v1/node-graphs/preview",
        "NodeGraphPreviewRequest",
        "EnginePreviewView",
    ),
    route(
        "job.submit",
        "POST",
        "/v1/jobs",
        "SubmitJobRequest",
        "JobView",
    ),
    route(
        "drift.compare",
        "POST",
        "/v1/drift/comparisons",
        "SubmitDriftComparisonRequest",
        "JobView",
    ),
    route(
        "job.read",
        "GET",
        "/v1/jobs/{objectId}",
        "ObjectIdRequest",
        "JobView",
    ),
    route(
        "job.list",
        "GET",
        "/v1/jobs",
        "ListJobsRequest",
        "JobPageView",
    ),
    route(
        "job.cancel",
        "POST",
        "/v1/jobs/{jobId}/cancel",
        "CancelJobRequest",
        "JobView",
    ),
    route(
        "export.submit",
        "POST",
        "/v1/exports",
        "SubmitExportRequest",
        "JobView",
    ),
    route(
        "export.read",
        "GET",
        "/v1/exports/{jobId}",
        "ObjectIdRequest",
        "JobView",
    ),
    route(
        "export.cancel",
        "POST",
        "/v1/exports/{jobId}/cancel",
        "CancelJobRequest",
        "JobView",
    ),
    route(
        "export.manifest.read",
        "GET",
        "/v1/exports/{exportId}/manifest",
        "ObjectIdRequest",
        "ExportManifestView",
    ),
    route(
        "export.files.list",
        "GET",
        "/v1/exports/{exportId}/files",
        "ListExportFilesRequest",
        "ExportFilePageView",
    ),
    route(
        "export.download",
        "GET",
        "/v1/exports/{exportId}/download",
        "ExportDownloadRequest",
        "ExportDownloadPage",
    ),
    route(
        "export.tombstone",
        "POST",
        "/v1/exports/{exportId}/tombstone",
        "TombstoneExportRequest",
        "ExportLifecycleView",
    ),
    route(
        "export.gc",
        "POST",
        "/v1/exports/gc",
        "CollectExportGarbageRequest",
        "ExportGarbageCollectionView",
    ),
    route(
        "run.read",
        "GET",
        "/v1/runs/{objectId}",
        "ObjectIdRequest",
        "RunView",
    ),
    route(
        "run.list",
        "GET",
        "/v1/runs",
        "ListRunsRequest",
        "RunPageView",
    ),
    route(
        "event.list",
        "GET",
        "/v1/events",
        "ListEventsRequest",
        "EventPageView",
    ),
    route(
        "audit.events.list",
        "GET",
        "/v1/audit/events",
        "ListAuditEventsRequest",
        "AuditEventPageView",
    ),
    route(
        "audit.lineage.read",
        "GET",
        "/v1/audit/lineage",
        "AuditLineageRequest",
        "AuditLineageView",
    ),
    route(
        "audit.export",
        "GET",
        "/v1/audit/export",
        "ListAuditEventsRequest",
        "AuditExportView",
    ),
    route(
        "artifact.read",
        "GET",
        "/v1/artifacts/{objectId}",
        "ObjectIdRequest",
        "ArtifactView",
    ),
    route(
        "artifact.list",
        "GET",
        "/v1/runs/{runId}/artifacts",
        "ListArtifactsRequest",
        "ArtifactPageView",
    ),
    route(
        "artifact.content",
        "GET",
        "/v1/artifacts/content",
        "ArtifactContentRequest",
        "ArtifactContentPage",
    ),
    route(
        "drift.report.read",
        "GET",
        "/v1/drift-reports/{artifactId}",
        "ObjectIdRequest",
        "ReportView",
    ),
    route(
        "quality.report.read",
        "GET",
        "/v1/quality-reports/{artifactId}",
        "ObjectIdRequest",
        "ReportView",
    ),
    route(
        "report.findings.list",
        "GET",
        "/v1/reports/{artifactId}/findings",
        "ListFindingsRequest",
        "FindingPageView",
    ),
    route(
        "automation.create",
        "POST",
        "/v1/automations",
        "CreateAutomationRequest",
        "AutomationView",
    ),
    route(
        "automation.list",
        "GET",
        "/v1/automations",
        "ListAutomationsRequest",
        "AutomationPageView",
    ),
    route(
        "automation.read",
        "GET",
        "/v1/automations/{automationId}",
        "AutomationIdRequest",
        "AutomationView",
    ),
    route(
        "automation.update",
        "POST",
        "/v1/automations/{automationId}",
        "UpdateAutomationRequest",
        "AutomationView",
    ),
    route(
        "automation.pause",
        "POST",
        "/v1/automations/{automationId}/pause",
        "AutomationTransitionRequest",
        "AutomationView",
    ),
    route(
        "automation.resume",
        "POST",
        "/v1/automations/{automationId}/resume",
        "AutomationTransitionRequest",
        "AutomationView",
    ),
    route(
        "automation.delete",
        "POST",
        "/v1/automations/{automationId}/delete",
        "AutomationTransitionRequest",
        "AutomationView",
    ),
    route(
        "automation.next-run",
        "GET",
        "/v1/automations/{automationId}/next-run",
        "AutomationIdRequest",
        "AutomationNextRunView",
    ),
    route(
        "automation.history.list",
        "GET",
        "/v1/automations/{automationId}/history",
        "ListAutomationHistoryRequest",
        "AutomationHistoryPageView",
    ),
    route(
        "automation.trigger",
        "POST",
        "/v1/automations/{automationId}/trigger",
        "TriggerAutomationRequest",
        "JobView",
    ),
];

pub const E5_A1_SCHEMAS: &[SchemaSpec] = &[
    schema("HandshakeRequest"),
    schema("HandshakeResponse"),
    schema("EmptyRequest"),
    schema("HealthView"),
    schema("MetricsView"),
    schema("CreateWorkspaceRequest"),
    schema("ArchiveWorkspaceRequest"),
    schema("WorkspaceView"),
    schema("CreateMemberRequest"),
    schema("MemberView"),
    schema("RevokeMemberRequest"),
    schema("CreateRoleRequest"),
    schema("RoleView"),
    schema("SetRoleCapabilitiesRequest"),
    schema("AssignRoleRequest"),
    schema("RoleAssignmentView"),
    schema("CreateServiceAccountRequest"),
    schema("ServiceAccountView"),
    schema("RevokeServiceAccountRequest"),
    schema("RegisterCredentialReferenceRequest"),
    schema("CredentialRefView"),
    schema("BeginCredentialRotationRequest"),
    schema("CompleteCredentialRotationRequest"),
    schema("RevokeCredentialRequest"),
    schema("RecoverCredentialRequest"),
    schema("CreateSessionRequest"),
    schema("SessionView"),
    schema("ListRequest"),
    schema("ObjectIdRequest"),
    schema("CloseSessionRequest"),
    schema("RegisterSourceConnectionRequest"),
    schema("SourceConnectionView"),
    schema("TestSourceConnectionRequest"),
    schema("ConnectionStatus"),
    schema("UpdateSourceConnectionRequest"),
    schema("TransitionSourceConnectionRequest"),
    schema("RetireSourceConnectionRequest"),
    schema("DiscoverAssetsRequest"),
    schema("InspectAssetRequest"),
    schema("AssetMetadata"),
    schema("PreviewAssetRequest"),
    schema("PreviewView"),
    schema("NodeCatalogView"),
    schema("ExpectedRevisionInput"),
    schema("SaveGraphRevisionRequest"),
    schema("GetGraphRevisionRequest"),
    schema("ListGraphRevisionsRequest"),
    schema("MigrateGraphRevisionRequest"),
    schema("GraphRevisionView"),
    schema("GraphRevisionSummary"),
    schema("GraphRevisionHistoryView"),
    schema("GraphRevisionMigrationView"),
    schema("CreateDatasetRequest"),
    schema("DatasetView"),
    schema("ListProfileHistoryRequest"),
    schema("ProfileHistoryEntryView"),
    schema("ProfileHistoryPageView"),
    schema("CreatePlanRequest"),
    schema("PlanView"),
    schema("SavePlanVersionRequest"),
    schema("PlanVersionView"),
    schema("ListPlanVersionsRequest"),
    schema("PublishPlanVersionRequest"),
    schema("ClonePlanRequest"),
    schema("PlanDiffRequest"),
    schema("PlanDiffView"),
    schema("ValidatePlanRequest"),
    schema("ValidatePlanView"),
    schema("EnginePreviewRequest"),
    schema("EnginePreviewView"),
    schema("NodeGraphCompileRequest"),
    schema("NodeGraphCompileView"),
    schema("NodeGraphPreviewRequest"),
    schema("SubmitJobRequest"),
    schema("SubmitDriftComparisonRequest"),
    schema("SubmitExportRequest"),
    schema("JobOperation"),
    schema("OperationKind"),
    schema("OperationDescriptorV1"),
    schema("SourceAssetRef"),
    schema("SnapshotRef"),
    schema("MaterializePolicyV1"),
    schema("VerificationPolicyV1"),
    schema("ProfileRequestV1"),
    schema("ProfileColumnsV1"),
    schema("ExportRequestV1"),
    schema("ExportDestinationV1"),
    schema("SnapshotOutputRef"),
    schema("ArtifactOutputRef"),
    schema("TerminalOutputRef"),
    schema("JobView"),
    schema("ListJobsRequest"),
    schema("JobPageView"),
    schema("CancelJobRequest"),
    schema("ExportManifestView"),
    schema("ExportInputView"),
    schema("ExportDestinationView"),
    schema("ExportFileView"),
    schema("ListExportFilesRequest"),
    schema("ExportFilePageView"),
    schema("ExportDownloadRequest"),
    schema("ExportDownloadPage"),
    schema("TombstoneExportRequest"),
    schema("ExportLifecycleView"),
    schema("CollectExportGarbageRequest"),
    schema("ExportGarbageCollectionView"),
    schema("RunView"),
    schema("ListRunsRequest"),
    schema("RunPageView"),
    schema("ListEventsRequest"),
    schema("EventPageView"),
    schema("ListAuditEventsRequest"),
    schema("AuditLineageRequest"),
    schema("AuditEventView"),
    schema("AuditEventPageView"),
    schema("AuditLineageView"),
    schema("AuditExportView"),
    schema("ArtifactView"),
    schema("ListArtifactsRequest"),
    schema("ArtifactPageView"),
    schema("ArtifactContentRequest"),
    schema("ArtifactContentPage"),
    schema("ReportView"),
    schema("ListFindingsRequest"),
    schema("FindingPageView"),
    schema("Vec<SourceAssetView>"),
    schema("ObjectList<SessionView>"),
    schema("ObjectList<SourceConnectionView>"),
    schema("ObjectList<SourceAssetView>"),
    schema("ObjectList<DatasetView>"),
    schema("ObjectList<PlanView>"),
    schema("ObjectList<PlanVersionView>"),
    schema("AutomationSchedule"),
    schema("AutomationRunTemplate"),
    schema("AutomationView"),
    schema("CreateAutomationRequest"),
    schema("ListAutomationsRequest"),
    schema("AutomationPageView"),
    schema("AutomationIdRequest"),
    schema("UpdateAutomationRequest"),
    schema("AutomationTransitionRequest"),
    schema("AutomationNextRunView"),
    schema("ListAutomationHistoryRequest"),
    schema("AutomationHistoryPageView"),
    schema("AutomationExecutionView"),
    schema("TriggerAutomationRequest"),
];

pub const E5_A1_MANIFEST: RouteManifest = RouteManifest {
    api_version: 1,
    routes: E5_A1_ROUTES,
    schemas: E5_A1_SCHEMAS,
};

pub const BOOTSTRAP_MANIFEST: RouteManifest = E5_A1_MANIFEST;

/// A mechanical OpenAPI-facing representation. A future listener can wrap
/// this value in transport-specific `info` and `servers` fields without
/// creating a second route or schema registry.
pub fn openapi_representation() -> Value {
    serde_json::to_value(E5_A1_MANIFEST).expect("static API manifest serializes")
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn every_manifest_route_references_known_schemas() {
        for route in E5_A1_ROUTES {
            assert!(E5_A1_SCHEMAS
                .iter()
                .any(|item| item.name == route.request_schema));
            assert!(E5_A1_SCHEMAS
                .iter()
                .any(|item| item.name == route.response_schema));
        }
    }

    #[test]
    fn openapi_representation_is_derived_from_manifest() {
        let json = openapi_representation();
        assert_eq!(json["apiVersion"], 1);
        assert_eq!(
            json["routes"].as_array().expect("routes").len(),
            E5_A1_ROUTES.len()
        );
    }
}
